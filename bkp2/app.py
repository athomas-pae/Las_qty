...
import welly.quality as q  # Importa las funciones de calidad correctamente
import os
...
def transpose_html_table(html):
    ...
    # Determine the number of columns in the original table
    num_cols = max(len(row.find_all(['td', 'th'])) for row in rows)

    # Create a list of lists to hold the new rows
    new_rows = [[] for _ in range(num_cols)]

    # Fill the new rows with the transposed data
    for row in rows:
        cells = row.find_all(['td', 'th'])
        for i, cell in enumerate(cells):
            new_rows[i].append(cell)

    # Create a new table and fill it with the new rows
...
def add_alias_column_to_html_table(html, alias_dict):
    ...
    # Add the header for the Alias column
    header = table.find('tr')
    alias_header = soup.new_tag('th')
    alias_header.string = 'Alias'
    header.append(alias_header)

    # Add the alias data to each row
    rows = table.find_all('tr')[1:]  # Skip the header row
    for row in rows:
        cells = row.find_all('td')
        curve_name = cells[0].text.strip()  # Assuming the curve name is in the first column
        detected_alias = 'N/A'
        ...
        if las:
            ...
            with col2:
                with st.expander("Resultados de las Pruebas de Calidad"):
                    # Crear la tabla HTML con los resultados de las pruebas de calidad usando Welly
                    tests = {
